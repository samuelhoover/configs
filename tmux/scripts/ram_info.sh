...
get_percent() {
  ...
  Darwin)
    used_memory=$(macmon pipe -s 1 -i 1000 | jq -r '(.memory.ram_usage / 1073741824 | round | tostring) + "GB"')
    total_memory=$(macmon pipe -s 1 -i 1000 | jq -r '(.memory.ram_total / 1073741824 | round | tostring) + "GB"')
